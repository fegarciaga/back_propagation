function [phi, O, w, invO_matrix_up, invO_matrix_dn, B_up, B_dn] = V(phi, phi_T, N_up, N_par, O, w, invO_matrix_up, invO_matrix_dn, B_up, B_dn, aux_fld, flag_bbp,N, list)
...
if w>0
    ...
    % stores random walk for back propagation measurement if needed
    for i=1:N+1
        if flag_bbp(i)==1
            B_up(list(i),i)=aux_fld(1,x_spin);
            B_dn(list(i),i)=aux_fld(2,x_spin);
        end
    end
